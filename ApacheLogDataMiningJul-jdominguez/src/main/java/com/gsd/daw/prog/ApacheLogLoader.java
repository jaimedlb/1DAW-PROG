...
public class ApacheLogLoader {
	...
	public static void main(String[] args) {
		...
		LOGGER.log(Level.INFO, "INFO: leidas [" + numeroLineasLeidas + "] lineas del fichero.");
		...
		LOGGER.log(Level.INFO, "creados [" + arrayLog.length + "] objetos del modelo.");
		...
		LOGGER.log(Level.INFO, "insertadas [" + (i - j) + "] filas en BBDD.");
	...
	private static Connection generarConexion(String ip, String estancia, String usuario, String contrasena) {
		...
		if (sgbd == null) {
			LOGGER.log(Level.INFO, "Se asume mySQL como base de datos");
			sgbd = SGBD_MYSQL;
		}
		switch (sgbd) {
		case SGBD_POSTGRESQL:
			conexionString = String.format("jdbc:postgresql://%s/%s", ip, estancia);
			LOGGER.log(Level.INFO, "Se asume POSTGRESQL como base de datos");
			break;
		case SGBD_ORACLE:
